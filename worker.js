...
export async function readD1ResearchDataset(DB){await ensureD1Schema(DB);const [raceResponse,predictionResponse,resultResponse]=await Promise.all([DB.prepare(`SELECT * FROM races ORDER BY updated_at DESC LIMIT 5000`).all(),DB.prepare(`SELECT race_id,model_version,COUNT(*) AS horse_count FROM predictions GROUP BY race_id,model_version`).all(),DB.prepare(`SELECT * FROM results ORDER BY race_id,finish,horse_no`).all()]),raceRows=raceResponse.results||[],predictionRows=predictionResponse.results||[],resultRows=resultResponse.results||[],predictionCounts=new Map(predictionRows.map(x=>[`${x.race_id}|${x.model_version}`,Number(x.horse_count)||0])),resultsByRace=new Map();for(const row of resultRows){const group=resultsByRace.get(row.race_id)||[];group.push(row);resultsByRace.set(row.race_id,group)}
 ...
 const records=[],exclusions=[],warnings=[],predictionRaceIds=new Set(),resultRaceIds=new Set(resultRows.map(x=>x.race_id));
 for(const [raceId,row] of selected){const prediction=parseJson(row.prediction_json),market=parseJson(row.market_json),final=parseJson(row.final_json),storedResult=parseJson(row.result_json),rebuiltResult=storedResult||reconstructedResultSnapshot(raceId,resultsByRace.get(raceId)),validation=parseJson(row.validation_json),predictionCount=prediction?.horses?.length||predictionCounts.get(`${raceId}|${row.model_version}`)||0,finishCount=rebuiltResult?.finishOrder?.length||0;if(predictionCount)predictionRaceIds.add(raceId);let reason='';if(!predictionCount)reason='PREDICTION_MISSING';else if(!rebuiltResult)reason='RESULT_MISSING';else if(!prediction?.horses?.length||!final?.top3?.length)reason='SNAPSHOT_INCOMPLETE';else if(finishCount<3)reason='DATA_QUALITY';if(reason)exclusions.push({raceId,modelVersion:row.model_version,reason,predictionCount,resultCount:resultsByRace.get(raceId)?.length||0,finishCount});if(prediction&&!prediction.integrity&&!prediction.fingerprint)warnings.push({raceId,reason:'LEGACY_UNVERIFIED'});records.push({raceId,eligible:!reason,exclusionReason:reason||null,reconstructedResult:!storedResult&&!!rebuiltResult,record:{race:parseJson(row.race_json)||prediction?.race||{},predictionSnapshot:prediction,marketSnapshot:market,finalSnapshot:final,resultSnapshot:rebuiltResult,validationSnapshot:validation,predictionCreatedAt:row.prediction_created_at,resultAcquiredAt:rebuiltResult?.fetchedAt||row.result_acquired_at,validationCompleted:finishCount>=3,validated:finishCount>=3,modelVersion:row.model_version,updatedAt:row.updated_at}})}
 const counts={d1PredictionSaved:predictionRaceIds.size,d1ResultFetched:resultRaceIds.size,validationEligible:records.filter(x=>x.eligible).length,resultMissing:[...predictionRaceIds].filter(id=>!resultRaceIds.has(id)).length,excluded:exclusions.length,totalRaces:selected.size};return {counts,records,exclusions,warnings}}
