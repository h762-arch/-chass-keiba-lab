 ...
 const u=new URL(request.url);
  ...
  if(u.pathname==="/api/nar/odds"||u.pathname==="/api/nar/sync"||u.pathname==="/api/nar/result-diagnostic"){
    ...
    try{
      ...
      if(u.pathname==="/api/nar/result-diagnostic"){try{const rf=await fetchNarResultResilient(urls.result,urls.resultIpat),parsed=rf.parsed||parseResult(rf.html),meta=parseRaceMeta(rf.html);const total=Array.isArray(parsed.results)?parsed.results.length:0,timeCount=Object.keys(parsed.actualTimes||{}).length,nameCount=(parsed.results||[]).filter(x=>x.horseName).length,detailCount=(parsed.results||[]).filter(x=>x.last3f!=null||x.cornerPositions||x.bodyWeight!=null).length;return json({ok:true,source:"NAR公式",version:VERSION,track:TRACK_NAMES[Number(code)]||"",code,date,race,resultStatus:parsed?.finishOrder?.length>=3?'available':'result_unpublished',finishOrder:parsed?.finishOrder?.slice(0,3)||[],results:Array.isArray(parsed?.results)?parsed.results:[],actualTimes:parsed?.actualTimes||{},resultMeta:{weather:meta?.weather||'',trackCondition:meta?.trackCondition||''},quality:{resultRows:total,resultParseRate:parsed?.parserFallback?50:total?100:0,actualTimeRate:total?Math.round(100*timeCount/total):0,resultHorseNameRate:total?Math.round(100*nameCount/total):0,resultDetailRate:total?Math.round(100*detailCount/total):0,parser:parsed?.parserFallback?'legacy-fallback':'header-mapped-v1'},urlType:rf.urlType,fallbackUsed:!!rf.fallbackUsed,fallbackTried:!!rf.fallbackTried,attemptCount:rf.totalAttemptCount||rf.attemptCount,diagnostics:rf.diagnostics||[],httpStatus:200,checkedAt:new Date().toISOString()})}catch(e){return json({ok:false,version:VERSION,track:TRACK_NAMES[Number(code)]||"",code,date,race,...errorPayload(e)},200)}}
