...
function scheduledNextCheck(attempts,nowMs){if(attempts>=AUTO_RESULT_MAX_ATTEMPTS)return new Date(nowMs+6*60*60000).toISOString();const i=Math.max(0,Math.min(AUTO_RESULT_RETRY_MINUTES.length-1,attempts-1));return new Date(nowMs+AUTO_RESULT_RETRY_MINUTES[i]*60000).toISOString()}
