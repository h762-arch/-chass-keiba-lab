...
export function minimalJson(payload,status=200){let body;const started=Date.now();try{body=JSON.stringify(payload)}catch(error){const fallback=JSON.stringify({ok:false,status:'failed',stage:'serialize',errorCode:'serialize_failed',error:String(error?.message||error),checkedAt:new Date().toISOString()});return new Response(fallback,{status:500,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store"}})}const bytes=new TextEncoder().encode(body).length;if(payload?.audit)payload.audit.serializeMs=Date.now()-started;body=JSON.stringify({...payload,payloadBytes:bytes});return new Response(body,{status,headers:{"content-type":"application/json; charset=utf-8","cache-control":"no-store"}})}
...
export default{
 ...
 const u=new URL(request.url);
  ...
  if(u.pathname==="/api/nar/sync-minimal"){
    const workerStarted=Date.now(),code=u.searchParams.get("code"),date=u.searchParams.get("date"),race=u.searchParams.get("race");if(!code||!date||!race)return minimalJson({ok:false,status:'failed',stage:'request_validate',errorCode:'invalid_payload'},400);
