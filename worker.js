...
export default{
 ...
 const u=new URL(request.url);
  ...
  if(u.pathname==="/api/nar/race"||u.pathname==="/api/nar/race-diagnostic"){
    ...
    try{
      const payload=await buildNarRacePayload({code,date,race,urls});if(u.pathname.endsWith('race-diagnostic'))payload.stage='race_diagnostic_complete';return json(payload);
  ...
  if(u.pathname==="/api/nar/odds"||u.pathname==="/api/nar/sync"||u.pathname==="/api/nar/result-diagnostic"){
    ...
    try{
      ...
      let oo=[];
      try{const oh=await fetchText(urls.odds);oo=parseTanFuku(oh)}catch(e){optionalErrors.push({stage:'odds',errorCode:'odds_fetch_failed',message:String(e?.message||e).slice(0,160)})}
