...
function requiredJraHorseNo(horse={},index=0){const no=numberOrNull(horse.horseNo??horse.horse_no);if(Number.isInteger(no)&&no>=1&&no<=99)return no;const name=text(horse.horseName??horse.horse_name??horse.name)||`row${index+1}`;throw new Error(`JRA_HORSE_NO_REQUIRED:${name}`)}
