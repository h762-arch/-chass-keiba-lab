...
await import('../src/research/longshot-scenario.js');
...
test('browser integration stores shadow data without replacing current longshot logic',async()=>{const [app,index]=await Promise.all([readFile(new URL('../app.js',import.meta.url),'utf8'),readFile(new URL('../index.html',import.meta.url),'utf8')]);assert.match(index,/jra-browser-bootstrap\.mjs/);assert.match(await readFile(new URL('../src/prediction/jra-browser-bootstrap.mjs',import.meta.url),'utf8'),/longshot-scenario\.js/);assert.match(index,/CHASS LONGSHOT SCAN/);assert.match(app,/buildLongshotScenario/);assert.match(app,/function evaluateLongshots/);assert.doesNotMatch(app,/longshotScenario[^\n]*\.win\s*=/)});
