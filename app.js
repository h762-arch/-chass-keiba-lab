(() => {
'use strict';
let jraMeetingSelector=null,jraRaceClient=null;
...
function setRaceMode(mode,{announce=true,preserveState=false}={}){const jra=mode==='JRA',changed=raceTypeOf(state)!==mode;if(changed&&!preserveState)beginActiveRaceTransition(mode);else if(changed)activeRaceGeneration++;if($('raceMode'))$('raceMode').value=jra?'JRA':'NAR';if($('narInputPanel'))$('narInputPanel').hidden=jra;if($('jraInputPanel'))$('jraInputPanel').hidden=!jra;if($('jraActualFields'))$('jraActualFields').hidden=!jra;if($('legacyJsonImport'))$('legacyJsonImport').hidden=jra;if($('raceModeStatus'))$('raceModeStatus').textContent=jra?'中央競馬 J2｜公式出馬表・手動Fallback':'地方競馬 Ver.9.9.35互換モード';if($('category'))$('category').value=jra?'中央競馬':'地方競馬';const marketCard=document.querySelector('.market-card'),autoOddsRow=$('autoOdds')?.closest?.('.switch-row');if(marketCard)marketCard.hidden=jra;if(autoOddsRow)autoOddsRow.hidden=jra;if($('liveOddsSync')){$('liveOddsSync').disabled=jra;$('liveOddsSync').textContent=jra?'JRAオッズ自動取得はJ3対象':'NAR公式から現在オッズを取得'}if($('narSync')){$('narSync').disabled=jra;$('narSync').textContent=jra?'JRA結果自動取得はJ4対象':'NAR公式から結果取得・自動保存'}jraMeetingSelector?.setActive(jra,{discover:!preserveState});jraRaceClient?.setActive(jra);if(announce&&jra&&$('jraStatus'))$('jraStatus').textContent='日付・競馬場・Rを選び、JRA公式データを取得してください。取得できない場合はJSON・CSV／手動入力を利用できます。';if(changed&&!preserveState)render()}
...
async function runAutoResultQueue({limit=1,allowHistoricalBoundary=false}={}){
 ...
 autoResultRunInProgress=(async()=>{let processed=0,completed=0,retry=0;renderAutoResultQueue();for(const [id,original] of dueResultQueue().slice(0,limit)){if(resultFetchInProgress||raceLoadController||liveOddsController)break;const record=raceCache[id]||original,code=narCode(record.resultQueue?.track||record.race?.track);if(!code)continue;try{const q=record.resultQueue,res=await fetchJsonSimple(`/api/nar/sync?code=${code}&date=${encodeURIComponent(q.raceDate)}&race=${q.raceNo}`,{timeoutMs:10000}),data=res.data;processed++;if(data.finishOrder?.length>=3){applyAutoFetchedResult(id,record,data);completed++}else{updateQueuedRecord(id,record,resultQueueRetryPatch(record,{kind:'unpublished',errorCode:'result_unpublished'}));retry++}}catch(error){processed++;updateQueuedRecord(id,record,resultQueueRetryPatch(record,{kind:'network',errorCode:error?.code||'network_error'}));retry++}await new Promise(resolve=>setTimeout(resolve,250))}return {processed,completed,retry}})().finally(()=>{autoResultRunInProgress=null;renderAutoResultQueue()});return autoResultRunInProgress
}
...
applyMarketOdds=function(items,acquiredAt){return applyMarketOddsBase(items,acquiredAt)};
async function syncLiveOdds(silent=false){
 if(raceTypeOf(state)==='JRA'){
  if($('oddsStatus'))$('oddsStatus').textContent='JRA Phase 1は入力ファイル内のオッズを使用します。';
  return {ok:false,status:'jra_manual_odds'};
 }
 if(autoResultRunInProgress)await autoResultRunInProgress;
...
saveFetchedResult=function(finishOrder,options={}){const ok=saveFetchedResultBase(finishOrder,options);if(ok){const horses=state.resultSnapshot?.horses||[],at=state.resultSnapshot?.fetchedAt||new Date().toISOString();applyScratchStatuses(state,horses,at,{source:'NAR_RESULT_STATUS'});applyScratchStatuses(state,horses.filter(h=>Number(h?.position??h?.finish)>0||!!h?.time||!!h?.actualTime||!!h?.cornerPositions),at,{source:'NAR_RESULT',resultAuthority:true});if(state.validationSnapshot)state.validationSnapshot.scratchEvaluation=buildScratchValidation(state);persist(true)}return ok};
function syncNar(options={}){
 if(options instanceof Event)options={};
 if(raceTypeOf(state)==='JRA'){
  if($('narStatus'))$('narStatus').textContent='JRA Phase 1の結果は手動入力またはファイル読込で保存してください。';
  return Promise.resolve({complete:false,pending:false,status:'jra_manual_result'});
 }
...
jraMeetingSelector=window.CHASS_JRA_MEETING?.create({isActive:()=>raceTypeOf(state)==='JRA',getGeneration:()=>activeRaceGeneration,onSelection:()=>{$('jraRaceNo')?.dispatchEvent(new Event('input',{bubbles:true}));jraRaceClient?.selectionChanged();}});
...
if($('quickPredict'))$('quickPredict').onclick=()=>raceTypeOf(state)==='JRA'?(jraDraft?$('jraAnalyze')?.click():$('jraOfficialLoad')?.click()):$('autoRaceLoad')?.click();
if($('quickOdds'))$('quickOdds').onclick=()=>{if(raceTypeOf(state)==='JRA'){if($('jraStatus'))$('jraStatus').textContent='J2では自動オッズ取得を行いません。入力ファイル内のオッズを使用します。';return}const card=document.querySelector('.market-card');if(card)card.open=true;$('liveOddsSync')?.click();requestAnimationFrame(()=>card?.scrollIntoView({behavior:'smooth',block:'start'}))};
