...
let cloudResearchAudit={counts:null,exclusions:[],missingResults:[],warnings:[],conflicts:[],loadedAt:null};
...
async function restoreResearchFromCloud(){const data=await cloudRequest('/api/db/research'),before=Object.keys(raceCache).length;cloudResearchAudit={counts:data.counts||null,exclusions:data.exclusions||[],missingResults:data.missingResults||[],warnings:data.warnings||[],conflicts:[],loadedAt:new Date().toISOString()};let added=0,enriched=0;for(const item of data.records||[]){if(!item?.raceId||!item.record)continue;const old=raceCache[item.raceId],oldFingerprint=old?researchContentFingerprint(old):null,merged=mergeCloudResearchRecord(old,item.record,item);if(merged.cloudConflict)cloudResearchAudit.conflicts.push({raceId:item.raceId,modelVersion:item.record?.modelVersion,reason:'DUPLICATE_OR_CONFLICT'});raceCache[item.raceId]=merged;if(!old)added++;else if(researchContentFingerprint(merged)!==oldFingerprint)enriched++}if(researchDb)await idbPutMany('races',Object.entries(raceCache).map(([id,record])=>({id,data:record,updatedAt:stableRecordUpdatedAt(record)})));else localStore.set(KEY,raceCache);renderCloudDatasetSummary();return {added,enriched,total:Object.keys(raceCache).length,before,counts:data.counts||{}}}
...
async function initCloudResearch(){
 ...
 try{
  ...
  const manifest=await cloudRequest('/api/db/manifest'),remoteManifest=new Map((manifest.records||[]).map(item=>[manifestKey(item),item])),data=await cloudRequest('/api/db/research');cloudResearchAudit={counts:data.counts||null,exclusions:data.exclusions||[],missingResults:data.missingResults||[],warnings:data.warnings||[],conflicts:[],loadedAt:new Date().toISOString()};
