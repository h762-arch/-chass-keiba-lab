...
function render(){
 ...
 $('integrityGrid').innerHTML=[['レースID',rid||'—'],['AI確率',`${prob}/${h.length}頭`],['予想TIME',`${time}/${h.length}頭`],['市場',`${market}/${h.length}頭`],['データ状態',bad?'要確認':'正常']].map(([a,b])=>`<div><span>${a}</span><strong>${esc(b)}</strong></div>`).join('');
...
function narCode(track){return {'船橋':19,'笠松':22,'園田':27,'姫路':28,'門別':36,'盛岡':10,'水沢':11,'浦和':18,'大井':20,'川崎':21,'金沢':23,'名古屋':24,'高知':31,'佐賀':32}[track]||null}
