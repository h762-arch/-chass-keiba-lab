...
function currentPublicDayApiUrl(){const context=activeRaceContext();if(!context.hasPrediction)return null;const race=state.race,date=String(race.raceDate||race.date||'').replaceAll('/','-'),track=String(race.track||'');if(!/^\d{4}-\d{2}-\d{2}$/.test(date)||!track)return null;const url=new URL('/api/chass/v1/public/day-ai',window.location.origin);url.searchParams.set('date',date);url.searchParams.set('track',track);url.searchParams.set('organization',context.organization);return url.toString()}
