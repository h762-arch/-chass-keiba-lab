...
function renderResultDisplayState(){
 ...
 if($('narStatus')&&!$('narStatus').textContent){
   $('narStatus').textContent=resultState.done?'公式結果を取得済みです。':resultState.status==='error'||resultState.status==='retry'?'再取得待ち｜一時的に公式結果を確認できませんでした。':'結果待ち｜レース終了後に公式結果を取得できます。';
 }
...
function makeSnapshot(){
 const now=new Date().toISOString(),ranked=rankFinal(),modelVersion=state.race?.jraModelVersion||APP_VERSION;
...
function render(){
 ...
 const liveConfidence=liveLayerActive()&&predictionViewMode==='live'?state.liveAdjustedPrediction.raceConfidence:r.raceConfidence;if($('confidenceBadge'))$('confidenceBadge').textContent=liveConfidence==null?'信頼度 —':`予想信頼 ${liveConfidence}%`;
 ...
 $('integrityGrid').innerHTML=[['レースID',rid||'—'],['競馬種別',raceTypeOf(state)],['AI確率',`${prob}/${h.length}頭`],['予想TIME',`${time}/${h.length}頭${availability.timeMissing?`（不足${availability.timeMissing}）`:''}`],['市場',marketState.label],['データ状態',bad?'要確認':'正常'],['保存方式',cloudState.available?'クラウド D1':researchStorageMode.includes('indexedDB')?'IndexedDB':'互換保存'],['生成方式',raceTypeOf(state)==='JRA'?'JRA入力':r.autoGenerated?'NAR自動':'JSON'],['評価モード',r.dataMode||'—']].map(([a,b])=>`<div><span>${a}</span><strong>${esc(b)}</strong></div>`).join('');
...
function renderFinal(){
 ...
 const picks=rankFinalFor(h.filter(isEligibleHorse)).slice(0,3), marks=['◎','○','▲']; const diamond=h.filter(x=>isEligibleHorse(x)&&x.valueMark).sort((a,b)=>(b.longshotScore||0)-(a.longshotScore||0)||(b.ev||0)-(a.ev||0))[0]; const warn=h.filter(x=>isEligibleHorse(x)&&x.warningMark).sort((a,b)=>(a.ev||999)-(b.ev||999))[0];
...
function setRaceMode(mode,{announce=true}={}){const jra=mode==='JRA';if($('raceMode'))$('raceMode').value=jra?'JRA':'NAR';if($('narInputPanel'))$('narInputPanel').hidden=jra;if($('jraInputPanel'))$('jraInputPanel').hidden=!jra;if($('raceModeStatus'))$('raceModeStatus').textContent=jra?'中央競馬 Phase 1｜手動・JSON・CSV入力':'地方競馬 Ver.9.9.35互換モード';if($('category'))$('category').value=jra?'中央競馬':'地方競馬';if($('liveOddsSync')){$('liveOddsSync').disabled=jra;$('liveOddsSync').textContent=jra?'JRAオッズ自動取得はPhase 1対象外':'NAR公式から現在オッズを取得'}if($('narSync')){$('narSync').disabled=jra;$('narSync').textContent=jra?'JRA結果は手動入力・保存':'NAR公式から結果取得・自動保存'}if(announce&&jra&&$('jraStatus'))$('jraStatus').textContent='JRA自動取得は行いません。1レース分のJSON・CSVまたは手動データを読み込んでください。'}
