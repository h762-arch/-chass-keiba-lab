...
function transform(root){
 ...
 const s=horses.map(h=>h.scores.timeIndex??5), n=normalize(s);
...
function render(){
 ...
 $('integrityGrid').innerHTML=[['レースID',rid||'—'],['AI確率',`${prob}/${h.length}頭`],['予想TIME',`${time}/${h.length}頭`],['市場',`${market}/${h.length}頭`],['データ状態',bad?'要確認':'正常'],['生成方式',r.autoGenerated?'NAR自動':'JSON']].map(([a,b])=>`<div><span>${a}</span><strong>${esc(b)}</strong></div>`).join('');
...
function provisionalModelFromOfficial(root){
 ...
 horses.forEach(h=>{
   ...
   const neutral=1/n;
   const win=100*((mp!=null?mp:neutral)*0.86+neutral*0.14);
   h.aiWinRate=clamp(win,0.5,75);
 });
 const weights=horses.map(h=>Math.pow(Math.max(num(h.aiWinRate)||0.5,.01),.72));
 const ws=weights.reduce((a,b)=>a+b,0)||1;
 horses.forEach((h,i)=>{
   h.aiPlaceRate=clamp(300*weights[i]/ws,2,88);
   h.dataConfidence=num(h.dataConfidence)??(withOdds.length>=Math.max(2,Math.floor(n*.7))?46:32);
   h.reason=h.reason||(
     withOdds.length
      ?'NAR公式出馬表＋現在/最終単勝から暫定生成。指数・近走ラップ等の追加データ未反映のため、確率は市場寄りの初期推定です。'
      :'NAR公式出馬表から暫定生成。市場・指数・近走ラップ等が未取得のため、能力差は限定的な初期推定です。'
   );
 });
 root.race={...root.race,dataSource:'NAR公式自動生成',autoGenerated:true,autoModel:'official+market provisional'};
 return root;
}
...
async function syncNar(){
 ...
 try{
   const res=await fetch(u,{cache:'no-store'}),d=await res.json();if(!res.ok)throw new Error(d.error||'取得失敗');
   if(d.finishOrder?.length>=3){$('finish1').value=d.finishOrder[0];$('finish2').value=d.finishOrder[1];$('finish3').value=d.finishOrder[2]}
   if(d.actualTimes&&typeof d.actualTimes==='object'){state.actualTimes={...state.actualTimes,...d.actualTimes};state.horses.forEach(h=>{const t=state.actualTimes[String(h.horseNo)];if(t)h.actualTime=t;});}
   if(Array.isArray(d.odds)&&d.odds.length)applyMarketOdds(d.odds,d.acquiredAt||new Date().toISOString());
   render();$('narStatus').textContent=`NAR公式反映：着順 ${d.finishOrder?.join('-')||'未確定'} / オッズ ${d.odds?.length||0}頭 / 実走TIME ${Object.keys(d.actualTimes||{}).length}頭`;persist(existingValidated(raceId(r)));
 }catch(e){$('narStatus').textContent='取得失敗：'+e.message}
}
