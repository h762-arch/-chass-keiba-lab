...
function applyScratchStatuses(record,statusItems=[],at=new Date().toISOString(),history=[]){if(!record?.predictionSnapshot)return record;const originalFingerprint=snapshotFingerprint(record.predictionSnapshot),live=buildLiveAdjustedPrediction(record,statusItems,at,history);if(!live)return record;record.liveAdjustedPrediction=live;record.scratchAudit={schemaVersion:1,scratchDetectedAt:record.scratchAudit?.scratchDetectedAt||at,lastCheckedAt:at,statusByHorse:{...live.statusByHorse},scratchImpactScore:live.scratchImpactScore,originalPredictionFingerprint:originalFingerprint,timeline:[...(record.scratchAudit?.timeline||[]).slice(-8),{at,event:record.scratchAudit?'live_adjusted_updated':'scratch_detected',horseNos:live.excludedHorseNos}]};if(snapshotFingerprint(record.predictionSnapshot)!==originalFingerprint)throw new Error('original_snapshot_mutated');return record}
function buildScratchValidation(record){const live=record?.liveAdjustedPrediction,top3=new Set((record?.resultSnapshot?.finishOrder||record?.result?.finishOrder||[]).map(Number).slice(0,3));if(!live)return null;const inactive=new Set(live.excludedHorseNos.map(Number)),originalPicks=(record?.finalSnapshot?.top3||[]).map(x=>Number(x.horseNo)).filter(no=>!inactive.has(no)),livePicks=(live.finalSnapshot?.top3||[]).map(x=>Number(x.horseNo));return {schemaVersion:1,scratchAffected:true,scratchExcludedHorseNos:[...inactive],officialBasis:'original_snapshot',originalTop3Hits:originalPicks.filter(no=>top3.has(no)).length,liveTop3Hits:livePicks.filter(no=>top3.has(no)).length,originalDenominator:originalPicks.length,liveDenominator:livePicks.length,originalPredictionFingerprint:live.originalPredictionFingerprint}}
...
function liveLayerActive(record=state){return !!record?.liveAdjustedPrediction?.excludedHorseNos?.length}
...
function setRaceMode(mode,{announce=true}={}){const jra=mode==='JRA';if($('raceMode'))$('raceMode').value=jra?'JRA':'NAR';if($('narInputPanel'))$('narInputPanel').hidden=jra;if($('jraInputPanel'))$('jraInputPanel').hidden=!jra;if($('jraActualFields'))$('jraActualFields').hidden=!jra;if($('raceModeStatus'))$('raceModeStatus').textContent=jra?'中央競馬 Phase 1｜手動・JSON・CSV入力':'地方競馬 Ver.9.9.35互換モード';if($('category'))$('category').value=jra?'中央競馬':'地方競馬';const marketCard=document.querySelector('.market-card'),autoOddsRow=$('autoOdds')?.closest?.('.switch-row');if(marketCard)marketCard.hidden=jra;if(autoOddsRow)autoOddsRow.hidden=jra;if($('liveOddsSync')){$('liveOddsSync').disabled=jra;$('liveOddsSync').textContent=jra?'JRAオッズ自動取得はPhase 1対象外':'NAR公式から現在オッズを取得'}if($('narSync')){$('narSync').disabled=jra;$('narSync').textContent=jra?'JRA結果は手動入力・保存':'NAR公式から結果取得・自動保存'}if(announce&&jra&&$('jraStatus'))$('jraStatus').textContent='JRA自動取得は行いません。1レース分のJSON・CSVまたは手動データを読み込んでください。'}
...
function commitJraNormalized(normalized){const prepared=mergeJraRace(normalized),result=window.CHASS_JRA_ADAPTER.run(prepared);state=result.state;state.modelVersion=result.prediction.modelVersion;fillRace(state.race);predictionViewMode='live';makeSnapshot();state.predictionSaved=true;persist(false);render();if($('jraStatus')){$('jraStatus').textContent=`JRA ${state.horses.length}頭｜CHASS指数・確率計算完了｜AI勝率合計 ${result.prediction.quality.winProbabilityTotal.toFixed(2)}%｜TIME ${result.prediction.quality.withTime}/${state.horses.length}｜市場 ${result.prediction.quality.withMarket}/${state.horses.length}`}$('quickCard')?.scrollIntoView?.({behavior:'smooth',block:'start'});return result}
...
commitAutoRaceData=function(d,options={}){const result=commitAutoRaceDataBase(d,options);applyScratchStatuses(state,[...(d?.horses||[]),...(d?.odds||[])],d?.acquiredAt||new Date().toISOString(),getAllRaces());if(liveLayerActive()){predictionViewMode='live';render();persist(existingValidated(raceId(state.race)))}return result};
...
document.querySelectorAll('[data-prediction-layer]').forEach(button=>button.onclick=()=>{predictionViewMode=button.dataset.predictionLayer==='original'?'original':'live';render()});
if($('quickList')){
