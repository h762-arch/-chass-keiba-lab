...
function makeSnapshot(){
 ...
 if(!state.predictionSnapshot){
   ...
   state.predictionSnapshot={schemaVersion:3,featureSchemaVersion:2,modelVersion,generatedAt:now,createdAt:now,raceType:raceTypeOf(state),race:snapshotRace(),horses:state.horses.map(predictionHorse),axisModel:axes,volatility,jraPrediction:state.jraPrediction?cloneData(state.jraPrediction):null,locked:true};
 }
...
function render(){
 ...
 renderScratchLayer();renderMarketDisplayState();renderResultDisplayState();renderValidationSummary();renderCloudSyncState();renderFinal();renderValuePicks();renderQuick();renderHorses();renderHistoricalSimilarity(rid);requestHistoricalSimilarity(rid);
}
