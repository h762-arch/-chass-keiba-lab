...
function setRaceMode(mode,{announce=true,preserveState=false}={}){const jra=mode==='JRA',changed=raceTypeOf(state)!==mode;if(changed&&!preserveState)beginActiveRaceTransition(mode);else if(changed)activeRaceGeneration++;if($('raceMode'))$('raceMode').value=jra?'JRA':'NAR';if($('narInputPanel'))$('narInputPanel').hidden=jra;if($('jraInputPanel'))$('jraInputPanel').hidden=!jra;if($('jraActualFields'))$('jraActualFields').hidden=!jra;if($('legacyJsonImport'))$('legacyJsonImport').hidden=jra;if($('raceModeStatus'))$('raceModeStatus').textContent=jra?'中央競馬 Phase 1｜手動・JSON・CSV入力':'地方競馬 Ver.9.9.35互換モード';if($('category'))$('category').value=jra?'中央競馬':'地方競馬';const marketCard=document.querySelector('.market-card'),autoOddsRow=$('autoOdds')?.closest?.('.switch-row');if(marketCard)marketCard.hidden=jra;if(autoOddsRow)autoOddsRow.hidden=jra;if($('liveOddsSync')){$('liveOddsSync').disabled=jra;$('liveOddsSync').textContent=jra?'JRAオッズ自動取得はPhase 1対象外':'NAR公式から現在オッズを取得'}if($('narSync')){$('narSync').disabled=jra;$('narSync').textContent=jra?'JRA結果は手動入力・保存':'NAR公式から結果取得・自動保存'}jraMeetingSelector?.setActive(jra,{discover:!preserveState});if(announce&&jra&&$('jraStatus'))$('jraStatus').textContent='出馬表の自動取得は次段階です。1レース分のJSON・CSVまたは手動データを読み込んでください。';if(changed&&!preserveState)render()}
...
jraMeetingSelector=window.CHASS_JRA_MEETING?.create({isActive:()=>raceTypeOf(state)==='JRA',getGeneration:()=>activeRaceGeneration,onSelection:()=>$('jraRaceNo')?.dispatchEvent(new Event('input',{bubbles:true}))});
...
if($('quickPredict'))$('quickPredict').onclick=()=>raceTypeOf(state)==='JRA'?$('jraAnalyze')?.click():$('autoRaceLoad')?.click();
if($('quickOdds'))$('quickOdds').onclick=()=>{if(raceTypeOf(state)==='JRA'){if($('jraStatus'))$('jraStatus').textContent='JRA Phase 1は読み込んだオッズを使用します。自動オッズ取得は行いません。';return}const card=document.querySelector('.market-card');if(card)card.open=true;$('liveOddsSync')?.click();requestAnimationFrame(()=>card?.scrollIntoView({behavior:'smooth',block:'start'}))};
