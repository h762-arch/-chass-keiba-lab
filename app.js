...
function renderFinal(){
 ...
 $('finalBody').innerHTML=`<div class="final-grid">${picks.map((x,i)=>`<div class="final-pick"><strong>${marks[i]} ${x.horseNo}番 ${esc(x.horseName)}</strong><small>勝 ${x.win.toFixed(1)}% ｜ 複 ${x.place.toFixed(1)}%<br>総合 ${x.overall}/100 ｜ 期待 ${x.ev==null?'市場待ち':x.ev.toFixed(0)+'%'} ｜ AIフェア ${x.fair?x.fair.toFixed(1)+'倍':'—'}</small></div>`).join('')}</div><div class="flags"><div class="flag-box diamond">${diamond?`${diamond.valueMark} ${esc(diamond.valueType||'穴馬')}：${diamond.horseNo}番 ${esc(diamond.horseName)} ｜ 能力 ${diamond.overall} ｜ 期待 ${diamond.ev.toFixed(0)}%`:'💎 穴馬：能力下限を満たす市場乖離なし'}</div><div class="flag-box warning">${warn?`${warn.warningMark} 人気馬注意：${warn.horseNo}番 ${esc(warn.horseName)} ｜ 期待 ${warn.ev.toFixed(0)}%`:'⚠️ 人気馬リスク：強い該当なし'}</div></div>`;
}
