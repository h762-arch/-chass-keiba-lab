...
async function fetchNarMinimalApi(url,{signal,generation,isCurrent}={}){const audit={request:{success:false,stage:'request_start'},headers:{success:false},body:{success:false},json:{success:false},payload:{success:false},checkedAt:new Date().toISOString()};let response;try{response=await fetch(url,{cache:'no-store',signal});audit.request={success:true,stage:'response_headers'};audit.headers={success:true,httpStatus:response.status}}catch(error){const code=error?.name==='AbortError'?(signal?.aborted?'fetch_aborted':'fetch_timeout'):'fetch_failed',e=requestError(code,error?.message||'fetch failed');e.transportAudit=audit;throw e}if(generation!=null&&typeof isCurrent==='function'&&!isCurrent()){const e=requestError('stale_request','古い結果取得を破棄しました。');e.transportAudit=audit;throw e}let text;try{text=await response.text();audit.body={success:true,bodyBytes:new TextEncoder().encode(text).length}}catch(error){const e=requestError('response_body_failed',error?.message||'response body failed',response.status);audit.body={success:false,errorCode:e.code};e.transportAudit=audit;throw e}if(!text){const e=requestError('empty_payload','空のAPI応答です。',response.status);audit.body={success:false,bodyBytes:0,errorCode:e.code};e.transportAudit=audit;throw e}let data;try{data=JSON.parse(text);audit.json={success:true}}catch(error){const e=requestError('json_parse_failed','API応答JSONを解析できませんでした。',response.status);audit.json={success:false,errorCode:e.code};e.transportAudit=audit;throw e}if(!response.ok){const e=requestError(data?.errorCode||'http_error',data?.error||`HTTP ${response.status}`,response.status);audit.payload={success:false,errorCode:e.code};e.transportAudit=audit;throw e}if(!data||data.ok!==true||!Array.isArray(data.finishOrder)){const e=requestError('invalid_payload','結果APIの形式が不正です。',response.status);audit.payload={success:false,errorCode:e.code};e.transportAudit=audit;throw e}audit.payload={success:true,stage:'payload_validate'};return {res:response,data,transportAudit:audit}}
...
async function syncNar(options={}){
 ...
 try{
   ...
   const apiFetch=await fetchNarMinimalApi(u,{signal:controller.signal,generation,isCurrent}),res=apiFetch.res,d=apiFetch.data;
   ...
   state.resultFetchCheckedAt=new Date().toISOString();
   state.resultFetchAudit={resultFetchSuccess:false,transportAudit:e?.transportAudit||null,primarySyncAudit:{success:false,stage:e?.stage||'api_fetch',errorCode:state.resultErrorType,httpStatus:e?.status||null,route:'sync-minimal',attemptCount:1},recoveryAudit:null,postFetchAudit:{success:null,stage:'not_started'},checkedAt:state.resultFetchCheckedAt};
