...
function applyScratchStatuses(record,statusItems=[],at=new Date().toISOString()){if(!record?.predictionSnapshot)return record;const originalFingerprint=snapshotFingerprint(record.predictionSnapshot),previous=record?.scratchAudit?.statusByHorse||{},statusByHorse={...previous};for(const item of statusItems||[]){const no=Number(item?.horseNo),raw=item?.horseStatus??item?.statusText??item?.positionText,status=normalizeHorseStatus(raw);if(no&&raw!=null&&status!=='active'&&status!=='unknown')statusByHorse[no]=status}const inactiveNos=Object.entries(statusByHorse).filter(([,status])=>INACTIVE_HORSE_STATUSES.has(status)).map(([no])=>Number(no));if(!inactiveNos.length)return record;for(const h of record.horses||[]){const status=statusByHorse[Number(h.horseNo)];if(INACTIVE_HORSE_STATUSES.has(status)){h.horseStatus=status;h.runnerStatus=status;h.eligible=false;h.statusLabel=horseStatusLabel(status)}}record.scratchAudit={schemaVersion:2,mode:'status_only',scratchDetectedAt:record.scratchAudit?.scratchDetectedAt||at,lastCheckedAt:at,statusByHorse,inactiveHorseNos:inactiveNos,originalPredictionFingerprint:originalFingerprint,legacyLiveAdjustmentPresent:!!record.liveAdjustedPrediction,timeline:[...(record.scratchAudit?.timeline||[]).slice(-8),{at,event:record.scratchAudit?'runner_status_updated':'runner_status_detected',horseNos:inactiveNos}]};if(snapshotFingerprint(record.predictionSnapshot)!==originalFingerprint)throw new Error('original_snapshot_mutated');return record}
function buildScratchValidation(record){const statusByHorse=record?.scratchAudit?.statusByHorse||{},inactive=new Set(Object.entries(statusByHorse).filter(([,status])=>INACTIVE_HORSE_STATUSES.has(status)).map(([no])=>Number(no)));if(!inactive.size)return null;const top3=new Set((record?.resultSnapshot?.finishOrder||record?.result?.finishOrder||[]).map(Number).slice(0,3)),originalPicks=(record?.finalSnapshot?.top3||[]).map(x=>Number(x.horseNo)).filter(no=>!inactive.has(no));return {schemaVersion:2,scratchAffected:true,scratchExcludedHorseNos:[...inactive],officialBasis:'original_snapshot',mode:'status_only',originalTop3Hits:originalPicks.filter(no=>top3.has(no)).length,originalDenominator:originalPicks.length,originalPredictionFingerprint:record.scratchAudit.originalPredictionFingerprint||snapshotFingerprint(record?.predictionSnapshot)}}
...
applyAutoFetchedResult=function(id,record,data,now=new Date().toISOString()){const working=cloneData(record);applyScratchStatuses(working,data?.results||[],data?.acquiredAt||now,[]);const saved=applyAutoFetchedResultBase(id,working,data,now);if(saved?.validationSnapshot){saved.validationSnapshot.scratchEvaluation=buildScratchValidation(saved);saveRaceRecord(id,saved)}return saved};
...
commitAutoRaceData=function(d,options={}){const result=commitAutoRaceDataBase(d,options);applyScratchStatuses(state,[...(d?.horses||[]),...(d?.odds||[])],d?.acquiredAt||new Date().toISOString());predictionViewMode='original';if(state.scratchAudit){render();persist(existingValidated(raceId(state.race)))}return result};
...
function renderResearchSyncStatus(data){const target=$('researchSyncStatus');if(!target)return;const config=data?.driveConfig||{},counts=data?.counts||{},jra=data?.organizations?.JRA||{},nar=data?.organizations?.NAR||{},enabled=config.enabled?'ON':'OFF',configLabel=config.complete?'設定完了':config.errorCode||'未設定';target.innerHTML=`<div><strong>Google Drive ${enabled}</strong><span>構成 ${esc(configLabel)}</span></div><div class="research-sync-counts"><span>同期済 ${Number(counts.synced)||0}</span><span>待機 ${Number(counts.pending)||0}</span><span>再試行 ${Number(counts.retry_wait)||0}</span><span>失敗 ${Number(counts.failed)||0}</span></div><small>JRA 同期済 ${Number(jra.synced)||0}｜NAR 同期済 ${Number(nar.synced)||0}${data?.lastSuccessfulSync?`｜最終成功 ${esc(new Date(data.lastSuccessfulSync).toLocaleString('ja-JP'))}`:''}</small>`}
async function refreshResearchSyncStatus(){const target=$('researchSyncStatus');try{const response=await fetch('/api/db/research-sync',{cache:'no-store',credentials:'same-origin',headers:{accept:'application/json'}}),data=await response.json();if(!response.ok||!data?.ok)throw Object.assign(new Error(data?.error||`HTTP ${response.status}`),{status:response.status});renderResearchSyncStatus(data);return data}catch(error){if(target)target.textContent=`状態取得失敗｜${error?.status?`HTTP ${error.status}`:error?.message||'通信エラー'}`;return null}}
async function checkDriveConnection(){const target=$('researchSyncStatus');if(target)target.textContent='Google Drive接続確認中…';try{const response=await fetch('/api/db/research-sync/drive-check',{method:'POST',cache:'no-store',credentials:'same-origin',headers:{'content-type':'application/json'},body:'{}'}),data=await response.json(),connection=data?.connection;if(!response.ok||!connection?.ok)throw Object.assign(new Error(connection?.errorCode||data?.error||`HTTP ${response.status}`),{status:response.status});await refreshResearchSyncStatus();if(target)target.insertAdjacentHTML('beforeend','<p class="good">Google Drive 接続正常｜Root Folder確認済み</p>');return true}catch(error){if(target)target.textContent=`Google Drive 接続失敗｜${esc(error?.message||'通信エラー')}`;return false}}
...
applyMarketOdds=function(items,acquiredAt){const count=applyMarketOddsBase(items,acquiredAt);applyScratchStatuses(state,items,acquiredAt||new Date().toISOString(),getAllRaces());return count};
...
saveFetchedResult=function(finishOrder,options={}){if(options?.resultData?.results)applyScratchStatuses(state,options.resultData.results,options.resultData.acquiredAt||new Date().toISOString(),getAllRaces());const ok=saveFetchedResultBase(finishOrder,options);if(ok&&state.validationSnapshot){state.validationSnapshot.scratchEvaluation=buildScratchValidation(state);persist(true)}return ok};
...
if($('driveConnectionCheck'))$('driveConnectionCheck').onclick=checkDriveConnection;
...
async function bootApp(){
 ...
 await refreshResearchSyncStatus();
 migrateLegacy();
