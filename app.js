...
function render(){
 ...
 if($('marketSummary'))$('marketSummary').textContent=`市場 ${market}/${h.length||'—'}`;
...
function renderFinal(){
 ...
 $('finalBody').innerHTML=`<div class="final-grid">${picks.map((x,i)=>`<div class="final-pick"><div class="final-mark">${marks[i]}</div><div class="final-no">${x.horseNo}</div><div class="final-name">${esc(x.horseName)}</div><div class="final-metric"><span>勝</span><b>${x.win.toFixed(1)}%</b></div><div class="final-metric"><span>複</span><b>${x.place.toFixed(1)}%</b></div><div class="final-metric"><span>TIME</span><b>${esc(x.predictedTime||'—')}</b></div><div class="final-metric"><span>総合</span><b>${x.overall}</b></div></div>`).join('')}</div><div class="flags"><div class="flag-box diamond">${diamond?`${diamond.valueMark} ${esc(diamond.valueType||'穴馬')}：${diamond.horseNo}番 ${esc(diamond.horseName)} ｜ 能力 ${diamond.overall} ｜ 期待 ${diamond.ev.toFixed(0)}%`:'💎 穴馬：能力下限を満たす市場乖離なし'}</div><div class="flag-box warning">${warn?`${warn.warningMark} 人気馬注意：${warn.horseNo}番 ${esc(warn.horseName)} ｜ 期待 ${warn.ev.toFixed(0)}%`:'⚠️ 人気馬リスク：強い該当なし'}</div></div>`;
}
...
function renderQuick(){
 ...
 $('quickList').innerHTML=`<div class="quick-table-head"><span>馬番</span><span>印・馬名</span><span>勝</span><span>複</span><span>TIME</span><span>総合</span></div>`+rows.map((x,i)=>`<div class="quick-row${i>=4?' is-extra':''}" data-horse-no="${x.horseNo}" role="button" tabindex="0" aria-label="${esc(x.horseName)}の詳細分析を開く"><div class="quick-no">${x.horseNo}</div><div class="quick-name">${esc(displayMark(x))} ${esc(x.horseName)}</div><div class="quick-stat"><span>勝</span><strong>${x.win.toFixed(1)}%</strong></div><div class="quick-stat"><span>複</span><strong>${x.place.toFixed(1)}%</strong></div><div class="quick-stat"><span>TIME${x.predictedTimeType?`(${esc(x.predictedTimeType)})`:''}</span><strong>${esc(x.predictedTime||'—')}</strong></div><div class="quick-stat"><span>総合</span><strong>${x.overall}</strong></div></div>`).join('');
