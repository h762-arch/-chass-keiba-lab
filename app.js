...
function restoreSavedRace(record){const next=cloneData(record||emptyRaceState());next.actualTimes=next.resultSnapshot?.actualTimes||next.result?.actualTimes||next.actualTimes||{};return next}
...
function makeSnapshot(){
 const now=new Date().toISOString(),ranked=raceTypeOf(state)==='JRA'?[...state.horses].sort((a,b)=>(b.overall??0)-(a.overall??0)||(b.win??0)-(a.win??0)):rankFinal(),modelVersion=state.race?.jraModelVersion||APP_VERSION;
 ...
 if(!state.finalSnapshot||!state.validationCompleted)state.finalSnapshot={schemaVersion:2,modelVersion,generatedAt:now,createdAt:now,marketType:state.race.oddsType,oddsSnapshotType:state.race.oddsSnapshotType||'unknown',top3,ranking:ranked.map((h,i)=>({...top3.find(x=>x.horseNo===Number(h.horseNo)),horseNo:Number(h.horseNo),horseName:h.horseName,rank:i+1,finalScore:finalScore(h),win:h.win,place:h.place,overall:h.overall,ev:h.ev,predictedTime:h.predictedTime}))};
 ...
 if(!state.predictionSnapshot){
   ...
   state.predictionSnapshot={schemaVersion:3,featureSchemaVersion:2,modelVersion,generatedAt:now,createdAt:now,raceType:raceTypeOf(state),race:snapshotRace(),horses:state.horses.map(predictionHorse),axisModel:axes,volatility,longshotScenario,jraPrediction:state.jraPrediction?cloneData(state.jraPrediction):null,locked:true};
 }
 state.snapshotSchemaVersion=state.predictionSnapshot?.schemaVersion||2;
 if(!state.marketSnapshot||!state.validationCompleted){state.marketSnapshot={schemaVersion:2,modelVersion,acquiredAt:state.race.oddsUpdatedAt||now,createdAt:state.race.oddsUpdatedAt||now,oddsSnapshotType:state.race.oddsSnapshotType||'unknown',horses:state.horses.map(marketHorse)}}
...
function render(){
 setVersion(); state.race=raceFromForm(); const r=state.race,h=displayHorses(); const rid=raceId(r);
...
function currentLongshotScenario(){return state.predictionSnapshot?.longshotScenario||null}
...
function setRaceMode(mode,{announce=true,preserveState=false}={}){const jra=mode==='JRA',changed=raceTypeOf(state)!==mode;if(changed&&!preserveState){state=emptyRaceState(mode);predictionViewMode='original';jraNormalizedDraft=null;clearValidationTransient(state)}if($('raceMode'))$('raceMode').value=jra?'JRA':'NAR';if($('narInputPanel'))$('narInputPanel').hidden=jra;if($('jraInputPanel'))$('jraInputPanel').hidden=!jra;if($('jraActualFields'))$('jraActualFields').hidden=!jra;if($('raceModeStatus'))$('raceModeStatus').textContent=jra?'中央競馬 Phase 1｜手動・JSON・CSV入力':'地方競馬 Ver.9.9.35互換モード';if($('category'))$('category').value=jra?'中央競馬':'地方競馬';const marketCard=document.querySelector('.market-card'),autoOddsRow=$('autoOdds')?.closest?.('.switch-row');if(marketCard)marketCard.hidden=jra;if(autoOddsRow)autoOddsRow.hidden=jra;if($('liveOddsSync')){$('liveOddsSync').disabled=jra;$('liveOddsSync').textContent=jra?'JRAオッズ自動取得はPhase 1対象外':'NAR公式から現在オッズを取得'}if($('narSync')){$('narSync').disabled=jra;$('narSync').textContent=jra?'JRA結果は手動入力・保存':'NAR公式から結果取得・自動保存'}if(announce&&jra&&$('jraStatus'))$('jraStatus').textContent='JRA自動取得は行いません。1レース分のJSON・CSVまたは手動データを読み込んでください。';if(changed&&!preserveState)render()}
...
function currentPublicApiUrl(){const race=raceFromForm(),date=String(race.raceDate||race.date||'').replaceAll('/','-'),track=String(race.track||''),raceNo=Number(race.raceNo);if(!/^\d{4}-\d{2}-\d{2}$/.test(date)||!track||!Number.isInteger(raceNo))return null;const organization=raceTypeOf({race})==='JRA'?'JRA':'NAR',url=new URL('/api/chass/v1/public/race',window.location.origin);url.searchParams.set('date',date);url.searchParams.set('track',track);url.searchParams.set('race',String(raceNo));url.searchParams.set('organization',organization);url.searchParams.set('format','compact');return url.toString()}
async function checkPublicApi(){const status=$('publicApiStatus');if(status)status.textContent='接続確認中…';try{const response=await fetch('/api/chass/v1/public/health',{headers:{accept:'application/json'}}),body=await response.json();if(!response.ok||body?.ok!==true)throw Object.assign(new Error(body?.error?.code||`HTTP ${response.status}`),{status:response.status});if(status)status.textContent=`接続正常｜Read-Only｜API v${body.apiVersion||'1'}`}catch(error){if(status)status.textContent=`接続失敗｜${error?.status?`HTTP ${error.status}`:error?.message||'通信エラー'}`}}
...
if($('autoTrack'))$('autoTrack').onchange=e=>{
  ...
  const before=Number($('autoRaceNo')?.value)||null;renderMeetingSelector({message:before&&!selectorRaceNumbers(e.target.value).includes(before)?'開催会場変更に伴いレースを1Rへ変更しました。':''});
};
if($('autoRaceDate'))$('autoRaceDate').onchange=e=>{
  if($('raceDate'))$('raceDate').value=e.target.value;
  refreshMeetingSelector(e.target.value).catch(()=>{});
};
...
['category','raceDate','track','raceNo','distance','trackCondition','chaos','pace'].forEach(id=>$(id).addEventListener('input',()=>{const before=raceId(state.race),next=raceFromForm(),after=raceId(next);if(before&&after&&before!==after){clearValidationTransient(state);state.horses=[];state.predictionSnapshot=null;state.marketSnapshot=null;state.finalSnapshot=null;state.scratchAudit=null}state.race=next;render()}));
