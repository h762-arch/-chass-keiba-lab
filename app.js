...
function manifestMatches(local,remote){return !!remote&&['predictionFingerprint','marketFingerprint','finalFingerprint','resultFingerprint','validationFingerprint','liveFingerprint'].every(key=>(local[key]||null)===(remote[key]||null))&&local.status===remote.status}
...
async function runAutoResultQueue({limit=1,allowHistoricalBoundary=false}={}){
 ...
 autoResultRunInProgress=(async()=>{let processed=0,completed=0,retry=0;renderAutoResultQueue();for(const [id,original] of dueResultQueue().slice(0,limit)){if(resultFetchInProgress||raceLoadController||liveOddsController)break;const record=raceCache[id]||original,code=narCode(record.resultQueue?.track||record.race?.track);if(!code)continue;try{const q=record.resultQueue,res=await fetchJsonSimple(`/api/nar/sync?code=${code}&date=${encodeURIComponent(q.raceDate)}&race=${q.raceNo}`,{timeoutMs:10000}),data=res.data;processed++;if(data.finishOrder?.length>=3){applyAutoFetchedResult(id,record,data);completed++}else{updateQueuedRecord(id,record,resultQueueRetryPatch(record,{kind:'unpublished',errorCode:'result_unpublished'}));retry++}}catch(error){processed++;updateQueuedRecord(id,record,resultQueueRetryPatch(record,{kind:'network',errorCode:error?.code||'network_error'}));retry++}await new Promise(resolve=>setTimeout(resolve,250))}if(completed)refreshCloudResearchDataset({persist:true,render:true}).catch(()=>{});return {processed,completed,retry}})().finally(()=>{autoResultRunInProgress=null;renderAutoResultQueue()});return autoResultRunInProgress
}
