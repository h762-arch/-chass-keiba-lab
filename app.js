...
commitAutoRaceData=function(d,options={}){const result=commitAutoRaceDataBase(d,options);applyScratchStatuses(state,d?.horses||[],d?.acquiredAt||new Date().toISOString(),{source:'NAR_RACE_CARD',authoritativeActive:true});applyScratchStatuses(state,d?.odds||[],d?.acquiredAt||new Date().toISOString(),{source:'NAR_ODDS'});predictionViewMode='original';if(state.scratchAudit){render();persist(existingValidated(raceId(state.race)))}return result};
...
applyMarketOdds=function(items,acquiredAt){const count=applyMarketOddsBase(items,acquiredAt);applyScratchStatuses(state,items,acquiredAt||new Date().toISOString(),{source:'NAR_ODDS'});return count};
