...
function publicHeaders(){return {'content-type':'application/json; charset=utf-8','cache-control':'no-store','access-control-allow-origin':'*','access-control-allow-methods':'GET, HEAD, OPTIONS','access-control-allow-headers':'Content-Type, Accept','x-content-type-options':'nosniff'}}
function sendPublicJson(req,res,status,data){res.writeHead(status,publicHeaders());res.end(req.method==='HEAD'?'':JSON.stringify(data))}
async function localPublicApi(req,res,u){if(req.method==='OPTIONS'){res.writeHead(204,publicHeaders());return res.end()}if(!['GET','HEAD'].includes(req.method))return sendPublicJson(req,res,405,{ok:false,error:{code:'METHOD_NOT_ALLOWED',message:'Only GET, HEAD and OPTIONS are allowed.'}});if(u.pathname==='/api/chass/v1/public/health')return sendPublicJson(req,res,503,{ok:false,error:{code:'DATABASE_UNAVAILABLE',message:'Cloudflare D1 is unavailable in the local static server.'}});return sendPublicJson(req,res,503,{ok:false,error:{code:'DATABASE_UNAVAILABLE',message:'Public prediction endpoints require the Cloudflare D1 binding.'}})}
...
http.createServer(async(req,res)=>{
  ...
  if(u.pathname==='/api/jra/result'){const response=await handleJraResultRequest(new Request(u,{method:req.method}),process.env);res.writeHead(response.status,Object.fromEntries(response.headers));return res.end(await response.text());}
