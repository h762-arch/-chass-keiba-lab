...
function sendMinimal(res,status,payload){try{const initial=JSON.stringify(payload),payloadBytes=Buffer.byteLength(initial),body=JSON.stringify({...payload,payloadBytes});res.writeHead(status,{"content-type":"application/json; charset=utf-8","cache-control":"no-store",...cors});res.end(body)}catch(error){sendJson(res,500,{ok:false,status:'failed',stage:'serialize',errorCode:'serialize_failed',error:String(error?.message||error)})}}
