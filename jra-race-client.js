(() => {
  ...
  window.CHASS_JRA_RACE_CLIENT={create({isActive,getSelection,getGeneration,commit}){
    ...
    async function load(){
      ...
      try{const q=new URLSearchParams({date:selection.date,track:selection.track,race:String(selection.race)}),response=await fetch('/api/jra/race?'+q,{signal:controller.signal});const data=await response.json();if(own!==token||!active||!isActive()||generation!==getGeneration())return;if(!response.ok||!data.ok)throw Error(data.error||'JRA_OFFICIAL_UNAVAILABLE');const savedOfficial=data.source==='JRA_SAVED_OFFICIAL_BASE';if((!savedOfficial&&data.dataConfidence!=='high')||!data.quality?.raceParsed||data.quality.horseCount<2||data.quality.horseNameRate!==1||data.quality.weightRate!==1||(!savedOfficial&&data.quality.jockeyRate!==1))throw Error('JRA_PARSER_INCOMPLETE');commit(data,generation);const sourceLabel=data.source==='JRA_DRIVE_FRIDAY_BASE'?'Drive事前データ':data.source==='JRA_SAVED_OFFICIAL_BASE'?'JRA保存済み公式データ（ライブ項目未確認）':'JRA公式';status.textContent=`${sourceLabel} ${data.quality.activeHorseCount}/${data.quality.horseCount}頭｜予想計算完了`;try{window.dispatchEvent(new CustomEvent('chass:jra-race-ready',{detail:{date:selection.date,track:selection.track,race:Number(selection.race),generation}}))}catch{}}
