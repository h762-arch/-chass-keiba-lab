...
function compactRun(run={}){
  return {
    ...
    popularity:run.popularity??null,
    finish:run.finish??null,
    finishText:run.finishText??null,
    time:run.time??null,
    timeSec:run.timeSec??null,
    margin:run.margin??null,
    last3f:run.last3f??null,
    bodyWeight:run.bodyWeight??null,
    jockey:run.jockey??null,
    weightCarried:run.weightCarried??null
  };
}

function compactHistoryHorse(horse={},runsLimit=10){
  return {
    lineageCode:horse.lineageCode??null,
    runCount:horse.runCount??0,
    summary:horse.summary??{},
    runs:Array.isArray(horse.runs)?horse.runs.slice(0,runsLimit).map(compactRun):[],
    fetchedAt:horse.fetchedAt??null,
    cacheHit:Boolean(horse.cacheHit),
    fingerprint:horse.fingerprint??null
  };
}
...
async function buildNarRaceContext(request,env,ctx){
  ...
  if(organization!=='NAR'){
    return json({ok:false,error:'race_context_currently_supports_NAR_only',status:400},400);
  }
  if(!date||!track||!race||!code){
    return json({
      ok:false,
      error:'date,track,race and resolvable NAR track code are required',
      status:400,
      received:{date,track,race,code}
    },400);
  }
  ...
  try{abilityPayload=await abilityResponse.json()}catch{
    return json({ok:false,error:'ability_payload_invalid_json',status:502},502);
  }
  try{historyPayload=await historyResponse.json()}catch{
  ...
  if(!abilityResponse.ok||abilityPayload?.ok===false){
    return json({
      ...
      status:abilityResponse.status||502,
      ability:abilityPayload
    },abilityResponse.status||502);
  }
  if(!historyResponse.ok||historyPayload?.ok===false){
    return json({
      ok:false,
      error:'history_api_failed',
      status:historyResponse.status||502,
      abilityMeta:{
        apiVersion:abilityPayload?.apiVersion??null,
  ...
  const horses=horseNos.map(horseNo=>{
    ...
    return {
      ...
      ability:{
        abilityRank:a.abilityRank??null,
        score:a.score??null,
        winProb:a.winProb??null,
        top3Prob:a.top3Prob??null,
        predictedTime:a.predictedTime??null,
        predictedTimeSec:a.predictedTimeSec??null,
        mark:a.mark??null,
        runningStyle:a.runningStyle??null,
        distanceScore:a.distanceScore??null,
        courseScore:a.courseScore??null,
  ...
  return json({
    ok:true,
    apiVersion:'chass-race-context-v1',
    purpose:'chat-fast-analysis',
    organization:'NAR',
    date,
    track,
    code:String(code),
